...
const buildKycEmailHtml = (otpCode) => `
  <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f172a">
    <h2 style="margin:0 0 12px 0;">Verify Your Email</h2>
    <p style="margin:0 0 8px 0;">Your KYC email verification code is <strong>${otpCode}</strong>.</p>
    <p style="margin:0 0 8px 0;">It expires in ${env.kycOtpExpiryMinutes} minutes.</p>
    <p style="margin:0;">If you did not request this code, you can safely ignore this email.</p>
  </div>
`;
...
export const sendKycOtpEmail = async ({ email, otpCode, requestId }) => {
  const normalizedEmail = normalizeEmail(email);
  const code = String(otpCode || "");
  ...
  await sendEmail({
    to: normalizedEmail,
    subject: "Your KYC Email Verification Code",
    text: `Your KYC email verification code is ${code}. It expires in ${env.kycOtpExpiryMinutes} minutes.`,
    html: buildKycEmailHtml(code),
    requestId,
  });
};
